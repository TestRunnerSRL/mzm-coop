...
#Download Bizhawk
$url = "https://github.com/TASVideos/BizHawk/releases/download/2.6/BizHawk-2.6.zip"
$filename = "bizHawk-2.6.zip"
Invoke-WebRequest -Uri $url -OutFile $filename
#Unzip
$zip_file = $shell_app.namespace((Get-Location).Path + "\$filename")
$destination = $shell_app.namespace((Get-Location).Path + "\BizHawk-2.6")
...
Move-Item -Path .\bizhawk-co-op-dev\* -Destination .\BizHawk-2.6\
Remove-Item .\bizhawk-co-op-dev -Recurse

Move-Item -Path .\luasocket\mime -Destination .\BizHawk-2.6\
Move-Item -Path .\luasocket\socket -Destination .\BizHawk-2.6\
Move-Item -Path .\luasocket\lua\* -Destination .\BizHawk-2.6\Lua\
Move-Item -Path .\luasocket\lua5.1.dll -Destination .\BizHawk-2.6\dll\
